...
Packages = c("dplyr", "ggplot2","pastecs", "rcompanion", "reshape2", "Rmisc", "here", "tidyverse", "stringr", "readxl") 
lapply(Packages, library, character.only = TRUE)

raw_responses <- read_excel("results-survey331442.xlsx")
...
# ggplot(relevant_counts[c(1:3),], aes(x="researchers and devs", y=counts))+
#   geom_bar(aes(fill=categories), stat="identity", position="stack", width = .2)+
#   coord_flip()+labs(x="Researchers and devs")+scale_x_discrete(labels=NULL)+guides()+
#   geom_label(aes(y=sum(counts)-counts+2, label=counts), position="identity")
# 
# ggplot(relevant_counts, aes(categories, counts, fill=categories))+
#   geom_bar(data=relevant_counts[c(4:7),], aes(categories, counts, fill=categories), stat="identity", width = .4)+
#   coord_flip()

ggplot(relevant_counts, aes(categories, counts, fill=categories))+
  geom_bar(aes(fill=categories), stat="identity", position="dodge")+
  scale_x_discrete(labels=NULL)+
  labs(title="Q1 Sample user types", x=NULL, y=NULL)+
  #coord_flip()+labs()+
  theme_minimal()+
...
Q1_other
  ...
  # donut plot
  # CC_T$fraction = CC_T$count / sum(CC_T$count)
  # CC_T = CC_T[order(CC_T$fraction), ]
  # CC_T$ymax = cumsum(CC_T$fraction)
  # CC_T$ymin = c(0, head(CC_T$ymax, n=-1))
  # CC_T$category <- str_c(CC_T$category, ' (', CC_T$count, ')')
  
  # p_cat = ggplot(CC_T, aes(fill=category, ymax=ymax, ymin=ymin, xmax=4, xmin=3)) +
  #   geom_rect() +
  #   coord_polar(theta="y") +
  #   xlim(c(0, 4)) +
  #   theme_minimal() +
