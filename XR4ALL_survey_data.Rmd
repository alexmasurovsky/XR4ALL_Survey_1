...
relevant_counts$xpos <- c(4.9, 4.1, 4.0) # position of outer circle labels
...
# # plot
# ggplot(relevant_counts, aes(fill=categories, ymax=ymax, ymin=ymin, xmin=5, xmax=6))+
#   geom_rect()+
#   xlim(c(0,8))+ # modify thickness; larger range = smaller, thinner plot
#   coord_polar(theta="y", start=0)+ # comment this out to better visualize x-y plane
#   geom_rect(data=counts2, aes(xmin=1, xmax=2))+
#   scale_fill_brewer(palette = "Blues")+ #"Dark2")+
#   scale_color_brewer(palette = "Blues")+ #copy above
#   labs(title="User Types", x=NULL, y=NULL, fill="User type")+#caption=paste0("(completed full survey, n = ", n_completed, ")"))+
#   theme_void()+
#   theme(legend.position='none', plot.title = element_text(hjust=.3, size=18))+
#   #guides(fill = guide_legend(reverse = TRUE))+
#   # fiddle with the x position here to move labels in/out:
#   geom_text(aes(label=categories, x=8, y=(ymax+ymin)/2), size=4)+
#   geom_text(aes(label=counts, x=5.5, y=(ymax+ymin)/2), size=3.5, color="white")+
#   geom_text(data=counts2, aes(label=categories, x=3.2, y=(ymax+ymin)/2), size=3.5, color="Grey2")+
#   geom_text(data=counts2, aes(label=counts, x=1.5, y=(ymax+ymin)/2), size=3.5, color="Grey2")
...
ggplot(relevant_counts, aes(fill=categories, ymax=ymax, ymin=ymin, xmin=xmin_l, xmax=xmax_l))+
  geom_rect()+
  xlim(c(-2,8))+ # modify thickness; larger range = smaller, thinner plot
  coord_polar(theta="y", start=0)+ # comment this out to better visualize x-y plane
  geom_rect(data=counts2, aes(xmin=xmin_s, xmax=xmax_s))+
  scale_fill_brewer(palette = "Blues")+ #"Dark2")+
  scale_color_brewer(palette = "Blues")+ #copy above
  labs(title="User Types", x=NULL, y=NULL, fill="User type",
       caption=paste0("(completed full survey, n = ", n_completed, ")"))+
  theme_void()+
  theme(legend.position='none', plot.title = element_text(hjust=.3, vjust = -7, size=18),
  ...
  geom_text(data=counts2, aes(label=categories, x=2.8, y=(ymax+ymin)/2), size=2.5, color="Grey2")+
