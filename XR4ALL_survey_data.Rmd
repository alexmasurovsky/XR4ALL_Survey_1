...
# format: Excel, Headings: Question code, Responses: full answers, everything else: defaults
...
Q3_counts <- data.frame(categories = c("Make reactive Unity software", "Observe user reactions while using", "Analyze user reactions after use", "Improve measurement tools","Other"),
           counts = c(nrow(completed_first_page %>% filter(`Q03WhyInterested[SQ001]`=="Yes")),
                      ...
                      nrow(completed_first_page %>% filter(`Q03WhyInterested[SQ003]`=="Yes")),
                      nrow(completed_first_page %>% filter(`Q03WhyInterested[SQ004]`=="Yes")),
                      nrow(completed_first_page %>% filter(is.na(`Q03WhyInterested[other]`)==FALSE)))) #%>% arrange(counts)
Q3_counts$categories <- factor(Q3_counts$categories, levels=Q3_counts$categories)
...
ggplot(Q3_counts, aes(categories, counts, fill=categories))+
  ...
  coord_flip()+
  scale_x_discrete(labels=NULL, limits = rev(levels(Q3_counts$categories)))+ #reverse x axis
  #guides(fill= guide_legend(reverse = TRUE))+
...
ggplot(Q3_counts, aes(categories, counts, fill=categories))+
  ...
  x=NULL, fill=NULL,
  caption=paste0("n = ", nrow(completed_first_page %>% filter(`Q01UserType[SQ001]`=="Yes"))))+
  scale_x_discrete(limits = rev(levels(Q3_counts$categories)))+
  coord_flip()+
  theme(legend.position = "none")+
  scale_fill_brewer(palette="Dark2")+
geom_text(aes(label=counts, y=counts-2), color="white", size=3.5)
ggsave("what_feature_developers.jpg")
...
ggplot(Q6_total_counts, aes(categories, counts, fill=categories))+
  ...
  labs(title="Q6 - Types of XR applications", x=NULL, y=NULL, fill=NULL)+
...
ggplot(Q7_counts, aes(x, freq, fill=x))+
  ...
  scale_fill_brewer(palette = "Dark2")+
  coord_flip()+
  geom_text(aes(label=freq, y=freq-1), color="white", size=3.5)
...
ggplot(Q8_counts, aes(x, freq, fill=x))+
  ...
  coord_flip()+
  scale_x_discrete(labels=NULL, limits = rev(levels(Q8_counts$categories)))+ #reverse x axis

  geom_text(aes(label=freq, y=freq-1), vjust=1.2, color="white", size=3.5)
...
Q9 <- completed_full_survey %>% filter(is.na(Q08WhatElse)==FALSE) %>% select(id, `Q08WhatElse`)
...
Q9$Q08WhatElse
sink("q9_number_one_thing.txt")
write(Q9$Q08WhatElse)
sink()
